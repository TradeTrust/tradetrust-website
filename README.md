...
`NET` is used for setting the default network, setting it to `mainnet` uses the public Ethereum network and `Corda Enterprise` to use Corda Enterprise network. If it is not set it defaults to Ropsten testnet.
...
or
```bash
NET="Corda Enterprise" npm run dev
```
