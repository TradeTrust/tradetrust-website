...
import {
  fetchTokenTransfers,
  identifyTokenTransferEventsFunction,
} from "./fetchTokenTransfer";
...
describe("Fetch Token Transfers", () => {
  ...
  describe("fetch from token registry with token id", () => {
    it("should return parsed transfer logs in valid format", async () => {
      ...
      if (!tokenRegistry)
        throw new Error("Unable to connect to token registry: Test Failed");
  ...
  describe("identify transfer events type", () => {
    ...
    expect(identifyTokenTransferEvents(burnLogEvent)).toBe(
      "SURRENDER_ACCEPTED"
    );
    ...
    expect(identifyTokenTransferEvents(surrenderRejectLogEvent)).toBe(
      "SURRENDER_REJECTED"
    );
  });
});
