...
export function useGaslessNominate(contractOptions: ContractOptions, providerOrSigner: any, chainId?: string) {
    ...
    async (params: NominateParams) => {
      ...
      try {
        ...
        if (useGasless) {
          ...
          const tx = await nominate({ titleEscrowAddress, tokenRegistryAddress, tokenId }, providerOrSigner, params, {
            id: keyId ?? "",
          });
