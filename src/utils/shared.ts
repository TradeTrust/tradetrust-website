...
export type WrappedOrSignedOpenAttestationDocument =
...
export const getChainId = (
  ...
  const processChainId = (
    ...
    if (document.network) {
      ...
      if (
        AvailableBlockChains.includes(document.network.chain) &&
        document.network.chainId
      ) {
  ...
  if (utils.isWrappedV2Document(rawDocument)) {
    ...
    if (identityProofType === "DNS-DID" || identityProofType === "DID")
      return undefined;
    return processChainId(documentData);
  } else if (utils.isWrappedV3Document(rawDocument)) {
    // Check for DID, ignore chainId when its DID
    const identityProofType =
      rawDocument.openAttestationMetadata.identityProof.type;
    if (identityProofType === "DNS-DID" || identityProofType === "DID")
      return undefined;
