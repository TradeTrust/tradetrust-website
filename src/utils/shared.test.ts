...
describe("getChainId for v2 document", () => {
  ...
  it.each(["MATIC", "POL"])("should return the correct chainId for polygon amoy network with chain %s", (chain) => {
    const document = {
      ...invoiceV2,
      data: { ...invoiceV2.data, network: { chain, chainId: "80002" } },
    } as unknown as WrappedOrSignedOpenAttestationDocument;
...
describe("getChainId for W3C v2 document", () => {
  ...
  it.each(["MATIC", "POL"])(
    "should return the correct chainId when W3C v2.0 document has tokenNetwork with polygon amoy chainId and chain %s",
    (chain) => {
      const documentWithPolygon = {
        ...w3cV2Document,
        credentialStatus: {
          ...w3cV2Document.credentialStatus,
          tokenNetwork: { chain, chainId: "80002" },
        },
      };
