...
const mockNetworks: ChainInfoObject[] = [
  getChainInfo(ChainId.Goerli),
  getChainInfo(ChainId.PolygonMumbai),
];
const mockUnsupportedNetwork: ChainInfoObject[] = [
  getChainInfo(ChainId.PolygonMumbai),
];
...
describe("NetworkSelect", () => {
  it("should render unsupported network", () => {
    render(
      <ProviderContextProvider
        ...
        networks={mockUnsupportedNetwork}
      >
        <NetworkSelect />
      </ProviderContextProvider>
    );
  ...
  it("should render the default network as selected", async () => {
    render(
      <ProviderContextProvider
        defaultChainId={ChainId.PolygonMumbai}
    ...
    const selectedLabel = await screen.findByText(
      getChainInfo(ChainId.PolygonMumbai).label
    );
  ...
  it("should render the list of networks when clicked", async () => {
    ...
    const polygonLabels = await screen.findAllByText(
      getChainInfo(ChainId.PolygonMumbai).label
    );
    const goerliLabels = await screen.findAllByText(
      getChainInfo(ChainId.Goerli).label
    );
  ...
  it("should render the selected network name when user switches network", async () => {
    ...
    const polygonItem = screen.getByTestId(
      `network-select-dropdown-label-${ChainId.PolygonMumbai}`
    );
