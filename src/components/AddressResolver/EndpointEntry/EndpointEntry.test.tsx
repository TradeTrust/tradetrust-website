...
describe("EndpointEntry", () => {
  it("should validate endpoint and save when validation passes", async () => {
    ...
    await waitFor(() => expect(mockGetFeatures).toBeCalled());
    await waitFor(() => expect(mockOnUpdateEndpoint).toBeCalled());
    expect(mockGetFeatures).toBeCalledWith("https://example.com", "x-api-key", "KEY");
    expect(mockOnUpdateEndpoint).toBeCalledWith({
      name: "TEST",
