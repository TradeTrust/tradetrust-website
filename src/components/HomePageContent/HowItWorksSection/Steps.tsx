...
const Step: FunctionComponent<StepDetail> = (props) => {
  ...
  return (
    <div
      className={`flex flex-row flex-none w-full xs:w-1/2 lg:w-auto mt-8 justify-center  ${
        icon ? "" : "lg:ml-6"
      }`}
    >
      <div className="flex flex-col max-w-[11.5rem] min-w-[11.5rem] justify-center">
        ...
        {title && (
          <h5 className={`text-center ${icon ? "mt-4" : "mt-0"}`}>{title}</h5>
        )}
        <h6
  ...
  return (
    <div className="flex flex-col justify-center">
      {contentType !== ContentType.BENEFIT && (
        <h3 className="hidden text-lemon-500 text-center mt-12 lg:inline">
          {contentType}
        </h3>
      )}
