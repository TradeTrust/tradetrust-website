      ...
      >
        ...
        {details.steps && (
          <div className="flex flex-col justify-center">
            ...
            {/* <h3 className="text-lemon text-center mt-12">THEN</h3>
            <div className="flex flex-row">
              {details.steps.thenSteps.map((steps, index) => (
                <div className="flex flex-row mt-6">
                  <div className="flex flex-col justify-center md:max-w-xxxs">
                    <img className="max-w-xs max-h-10" src={steps.icon} />
                    <h6 className="text-center">Step {steps.stepNumber}</h6>
                    <h6 className="font-normal text-center">{steps.stepDescription}</h6>
                  </div>
                  {index + 1 != details.steps?.thenSteps.length && (
                    <div className="w-48 mt-8 border-dashed border-t border-white"></div>
                  )}
                </div>
              ))}
            </div>
            <h3 className="text-lemon text-center mt-12">NOW</h3>
            <div className="flex flex-row">
              {details.steps.nowSteps.map((steps, index) => (
                <div className="flex flex-row m-auto mt-6">
                  ...
                  {index + 1 != details.steps?.nowSteps.length && (
                    <div className="w-48 mt-8 border-solid border-t border-white"></div>
                  )}
                </div>
              ))}
            </div> */}
            <h5 className="text-center">{details.steps.endMessage}</h5>
          </div>
        )}
        {details.benefits && (
          <div className="flex flex-col justify-center">
