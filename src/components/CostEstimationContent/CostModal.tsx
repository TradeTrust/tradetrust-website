...
export const CostModal: FunctionComponent<CostModalProps> = ({ costData }) => {
  ...
  const generateDescription = useCallback(() => {
    return `*Estimations based on the current gas average at ${Math.ceil(
      eGwei
    )} gwei (ETH), ETH price at USD
      $${ePrice} for Ethereum and ${Math.ceil(
      mGwei
    )} gwei (MATIC), MATIC price at USD $${mPrice} for Polygon
     as at ${dateTime}.`;
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex flex-col lg:flex-row justify-around w-full max-w-5xl">
              {costs.map((cost, i) => {
                return (
                  <div
                    key={`cost-information-${i}`}
                    className="flex flex-col items-center mt-2 mb-6 mx-4"
                  >
                    <img
                      className="max-h-10 h-10 max-w-[3rem]"
                      src={cost.icon}
                      alt={`cost-info-${cost.title}`}
                    />
