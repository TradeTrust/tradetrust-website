...
export const CostHeader: FunctionComponent = () => {
  ...
  const { price: maticPrice, gwei: maticGwei } = useFetchGasPrice(
    "polygon",
    REFRESH_RATE
  );
      ...
      <div className="container">
        ...
        >
          <div className="text-center md:text-left w-full md:w-1/2 mt-4 lg:mt-24 mb-32 md:my-0 relative -top-20 md:-top-12 lg:-top-24">
            <h1>Costing</h1>
            <h3 className="mt-4">
              TradeTrust is free to use for all, and anyone can implement using
              TradeTrust source code!
            </h3>
            <h4 className="mt-2">
              Just pay for gas fees from as low as{" "}
              <FiatLabel>{`${lowestPrice}`}</FiatLabel> per transaction* to use
              the blockchain.
            </h4>
            <h6 className="text-cloud-400 mt-2">
              *Transaction with the lowest gas fee as at {dateTime}: Cost to
              transfer holdership (Polygon)
            </h6>
          </div>
        </div>
      </div>
