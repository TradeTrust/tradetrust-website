...
import { PartnersData, PartnerType, TypesOfPartners, PartnerDataSingle, PartnersDataProps } from "./types";

const MAX_PARTNER_TILES_TO_DISPLAY = 9;

export const getPartnersData = (type: PartnerType, allPartnersData: PartnerDataSingle[]): PartnersData[] => {
...
export const PartnersContent: FunctionComponent<PartnersDataProps> = ({ partnersData }) => {
  ...
  useEffect(() => {
    setAnyPlatformData(getPartnersData(TypesOfPartners.PLATFORM, partnersData).length > 0);
    setAnyPartnersData(getPartnersData(TypesOfPartners.PARTNER, partnersData).length > 0);
    const partnerData = getPartnersData(partnerState, partnersData);
  ...
  }, [currentPage, partnerState, partnersData]);
